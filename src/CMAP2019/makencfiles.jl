using NCTiles,NCDatasets,NetCDF,Dates,Printf,ClusterManagers,Distributed

run_parallel = haskey(ENV,"SLURM_NTASKS")
...
@everywhere using NCTiles,NCDatasets,NetCDF,Dates,Printf,DistributedArrays
...
@everywhere function createTimestep(timestep,idx,group,flds,timestart,fnameprefix,README,dimdict)
...
for group in groups
    ...
    if run_parallel
        dtsteps = distribute(collect(timesteps[timestepidx]))
        didx = distribute(collect(timestepidx))

        createTimestep.(dtsteps,didx,Ref(group),Ref(flds),Ref(timestart),Ref(ncdir*group*"/"*fnameprefix),Ref(README),Ref(dimdict))
   else
